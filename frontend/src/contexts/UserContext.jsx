...
function UserProvider({ children }) {
  ...
  async function getUserFromHisId(id) {
    ...
    const url = `https://backend-opal-sigma.vercel.app/api/user/getUserById/${id}`;
    try {
  ...
  async function UpdateUserFromHisId(id, formData) {
    const url = `https://backend-opal-sigma.vercel.app/api/user/editUserById/${id}`;
  ...
  async function getAllUsers() {
    const url = `https://backend-opal-sigma.vercel.app/api/user/allusers`;
    try {
      const res = await axios.get(url);
      if (res) {
